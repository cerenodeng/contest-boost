...
export default function AdminSearch({ originalContests, refreshContest, deleteItem }) {
  const sidebarNavs = ['contests', 'pages', 'users'];
  const pathname = usePathname();
  const lastSegment = pathname.split('/').at(-1);
  const searchClass = sidebarNavs.includes(lastSegment) ? '' : 'hidden';

  const [contests, setContests] = useState(originalContests);
        ...
        {contests.map((contest) => (
          <div className="flex py-5 group hover:text-black/70" key={contest.id}>
            ...
            <form className="w-1/4 invisible group-hover:visible">
              <button onClick={async () => deleteItem(contest.id)}>
